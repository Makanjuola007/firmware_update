 ...
 * ─────────────────────────────────────────────────────────────
 *
 * v6 changes (GitHub-hosted OTA firmware updates):
 *
 *  Repo: https://github.com/Makanjuola007/firmware_update
 *  Expected layout on that repo's OTA_GITHUB_BRANCH ("main"):
 *    ver.txt       -> plain text, ONLY the version string, e.g.
 *                     "1.0.1" (must match FW_VERSION exactly on
 *                     a match - any difference triggers an update).
 *    firmware.bin  -> the compiled binary for this exact sketch,
 *                     built with the SAME partition scheme as
 *                     what's currently flashed on the device.
 *
 *  checkForOTAUpdate() GETs ver.txt over HTTPS from
 *  raw.githubusercontent.com. If it differs from the FW_VERSION
 *  baked into this build, it streams firmware.bin straight into
 *  the inactive OTA partition via the HTTPUpdate library (no need
 *  to hold the whole binary in RAM) and reboots into it on success.
 *
 *  It is added as a 4th entry in CommsTask's existing
 *  "most-overdue-task-wins" scheduler (OTA_CHECK_INTERVAL_MS,
 *  default 1 hour) rather than its own task, for two reasons:
 *    1. This board has no PSRAM and can only reliably hold ONE
 *       TLS session open at a time (see configureSecureClient()
 *       below) - a separate OTA task would race CommsTask/
 *       StreamTask for that one slot.
 *    2. It's a big, slow, one-shot download - it belongs in the
 *       same serialized queue as the other infrequent network
 *       work (balance poll), not competing for priority with
 *       time-sensitive telemetry/relay traffic.
 *
 *  To ship an update: bump FW_VERSION below, build the new .bin,
 *  push it to the repo as firmware.bin, and update ver.txt on
 *  that branch to the same string as the new FW_VERSION. Every
 *  device polling that branch picks it up within one
 *  OTA_CHECK_INTERVAL_MS window.
 * ─────────────────────────────────────────────────────────────
 */
...
#include <HTTPUpdate.h>           // OTA (v6) - see checkForOTAUpdate() below
...
// ── OTA (v6) ─────────────────────────────────────────────────
// Bump FW_VERSION here AND write the identical string to ver.txt
// on OTA_GITHUB_BRANCH whenever you ship a new build. A mismatch
// (in either direction - older or newer string) triggers a
// re-flash from firmware.bin on that same branch.
#define FW_VERSION            "1.0.0"
#define OTA_GITHUB_USER       "Makanjuola007"
#define OTA_GITHUB_REPO       "firmware_update"
#define OTA_GITHUB_BRANCH     "main"
#define OTA_VERSION_URL   "https://raw.githubusercontent.com/" OTA_GITHUB_USER "/" OTA_GITHUB_REPO "/" OTA_GITHUB_BRANCH "/ver.txt"
#define OTA_BINARY_URL    "https://raw.githubusercontent.com/" OTA_GITHUB_USER "/" OTA_GITHUB_REPO "/" OTA_GITHUB_BRANCH "/firmware.bin"
#define OTA_CHECK_INTERVAL_MS  3600000UL   // 1 hour - it's a big blocking download, don't do it often
// ── END OTA ──────────────────────────────────────────────────
...
void commsTask(void* pv) {
  ...
  unsigned long last_ota_ms        = 0;   // OTA (v6)
  ...
  for (;;) {
    ...
    // OTA (v6) is entry #3: a long interval means it naturally
    // stays low-priority against the others without any extra
    // logic, and it still runs on this same task so it can never
    // open a second TLS session alongside CommsTask's own calls. ----
    ...
    NetTask tasks[4] = {
      ...
      { &last_ota_ms,        OTA_CHECK_INTERVAL_MS,      3 },   // OTA (v6)
    };
...
void pushTelemetryAndSystem() {
  ...
  n = snprintf(body + pos, sizeof(body) - pos,
    ...
    s_source.c_str(),
    s_rate,
    FW_VERSION            // OTA (v6): report running version so the
                           // dashboard can show it per-device
  );
  if (n > 0) pos += n;
...
//  and see checkForOTAUpdate() (v6) for why OTA deliberately runs
//  from CommsTask instead of its own task, for the same reason.
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  checkForOTAUpdate() — GitHub-hosted OTA (v6)
//  Repo: https://github.com/Makanjuola007/firmware_update
//
//  1. GET ver.txt from raw.githubusercontent.com over HTTPS.
//  2. If it differs from FW_VERSION, download firmware.bin with
//     the HTTPUpdate library, which streams the download straight
//     into the inactive OTA partition (no full-binary RAM buffer)
//     and validates it before switching boot partitions.
//  3. On success it reboots automatically (rebootOnUpdate(true))
//     into the new firmware; on failure it just logs and returns,
//     leaving the current firmware running untouched, and the
//     scheduler will simply try again next OTA_CHECK_INTERVAL_MS.
//
//  Deliberately called only from CommsTask's own loop (never a
//  separate task) - see the "Secure-client helper" comment above
//  for why this board can only afford one open TLS session.
// ─────────────────────────────────────────────
void checkForOTAUpdate() {
  if (WiFi.status() != WL_CONNECTED) { return; }
  if (!heapOKForHttp("OTA-check")) { return; }
  ...
  Serial.println("[OTA] Checking " OTA_VERSION_URL);
  ...
  if (status == 200) {
    ...
    Serial.printf("[OTA] version check HTTP status %d\n", status);
  }
  https.end();
  ...
  if (remoteVersion == FW_VERSION) {
    blinkReceived();   // checked OK, already up to date
    return;
  }

  Serial.println("[OTA] Version differs - downloading and flashing firmware.bin ...");
  blinkPattern(3, 80, 80);   // distinct pattern: "about to reflash"

  // https.end() above already closed the version-check session, so
  // only this one secure session is open for the download itself -
  // same one-TLS-session-at-a-time constraint as everywhere else.
  WiFiClientSecure otaClient;
  configureSecureClient(otaClient);

  httpUpdate.rebootOnUpdate(true);   // auto-reboots into new firmware on success
  t_httpUpdate_return result = httpUpdate.update(otaClient, OTA_BINARY_URL);
  ...
  switch (result) {
    ...
    case HTTP_UPDATE_NO_UPDATES:
      ...
      break;
    case HTTP_UPDATE_OK:
      Serial.println("[OTA] Update OK - rebooting into new firmware now.");
      // rebootOnUpdate(true) means execution may not reach past this line.
      break;
  }
}
